...
var token = 'babc9a5cd4f343f9be0c7bd9cf93600c'; // demo account
...
var checkLoggedInterval = function() {
    var checkLogged = function() {
        return (document.cookie.indexOf('user_uid=') >= 0);
    };

    checkLoggedInterval.interval = setInterval(function() {
        var logged = checkLogged();
        if (logged) {
            $('#login-status').html("your are logged, the model will be uploaded to your account");
            userIsLogged = true;
        } else {
            $('#login-status').html("your are not logged, the model will plublish on sculptfab account or you can <a href='https://sketchfab.com/login' target='_blank'> signup/signin to sketchfab.com</a>");
            userIsLogged = false;
        }
    }, 1000);
...
Sketchfab.showUploader = function(options, callback) {
    ...
    var onClick = function(evt) {
        ...
        if (!userIsLogged) { // dont use default token if user is logged
            options.token = token;
        }
