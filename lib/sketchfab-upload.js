...
var checkLoggedInterval = function() {
    ...
    checkLoggedInterval.interval = setInterval(function() {
        ...
        if (logged) {
            ...
            if ( ! $('.upload-button').hasClass('uploading') ) {
                $('.upload-button').text('Publish model');
            }
            userIsLogged = true;
            ...
            $('#login-status').html("<strong class='warning'>You are not logged into Sketchfab.<br>You need <a href='https://sketchfab.com/login' target='_blank'>login</a> to enter the EasterEgg Challenge.</strong>");
            if ( ! $('.upload-button').hasClass('uploading') ) {
                $('.upload-button').text('Publish to demo account anyway');
            }
            userIsLogged = false;
            token = demoToken;
        }
...
Sketchfab.showUploader = function(options, callback) {
    ...
    sheet.innerHTML = ".skfb-uploader { "
        ...
        + "}"
        + ".skfb-uploader .warning { color: #F60; }"
        + ".skfb-uploader .btn { }" ;
    document.body.appendChild(sheet);

    var formTpl = _.template("<h1>Publish to Sketchfab</h1>"
                             + "<p id='login-status'></p>"
                             + "<div><input type='text' value='<%= title %> EasterEgg' placeholder='Model name' class='js-data-title'></input></div>"
    ...
    var successTpl = _.template("<h1>Success!</h1>"
                                ...
                                + "<div><a href='http://twitter.com/intent/tweet?text=<%= tweet %>' target='_blank' class='btn share-twitter'>Share on Twitter</a></div>"
    ...
    var onUpload = function(err, data) {
        if(err || data.error !== undefined) {
            ...
            uploaderbox.innerHTML = successTpl({
                url : url,
                tweet : encodeURIComponent('I made a 3D Easter Egg on @Sketchfab: ' + url + '. Create one and win a 3D print by @Shapeways http://tmblr.co/ZpQmrs1CfFEgL #easteregg')
            });
            bindCancelButton();
