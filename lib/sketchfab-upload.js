...
Sketchfab.showUploader = function(options, callback) {
  ...
  sheet.innerHTML = ".skfb-uploader { "
                    ...
                    + "width:440px; height: 400px; margin-left: -235px; margin-top: -250px; padding: 35px 50px; "
                    + "background-color: #222; "
                  + "}"
                  + ".skfb-uploader .btn { }" ;
  document.body.appendChild(sheet);

  var formTpl = _.template("<h1>Export to Sketchfab</h1>"
                          + "<div><label>Title</label><input type='text' value='<%= title %>' placeholder='Model name' class='js-data-title'></input></div>"
                          + "<div><label>Description</label><textarea value='' placeholder='Model description (optional)' class='js-data-description'></textarea></div>"
                          + "<a class='cancel js-cancel' href='#'><i class='icon-remove'></i></a><a class='upload-button btn js-ok' href='#'>Upload</a>");
