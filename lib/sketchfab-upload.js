...
Sketchfab.showUploader = function(options, callback) {
    ...
    var formTpl = _.template("<h1>Publish to Sketchfab</h1>"
                             + "<p id='login-status'></p>"
                             + "<div><input type='text' value='<%= title %> Easter Egg' placeholder='Model name' class='js-data-title'></input></div>"
    ...
    var onUpload = function(err, data) {
        if(err || data.error !== undefined) {
            ...
            uploaderbox.innerHTML = successTpl({
                url : url,
                tweet : encodeURIComponent('I made a 3D Easter Egg on @Sketchfab: ' + url + '. Create one and win a 3D print by @Shapeways http://skfb.ly/eggpainter #easteregg')
            });
            bindCancelButton();
